...
contextBridge.exposeInMainWorld("centralDesktop", {
  ...
  setWhatsAppVisible: (visible) => ipcRenderer.invoke("whatsapp:set-visible", visible),
  ...
  onProfile: (callback) => {
    ...
    ipcRenderer.on("whatsapp:profile", listener);
    return () => ipcRenderer.removeListener("whatsapp:profile", listener);
  },
  onUnread: (callback) => {
    const listener = (_event, payload) => callback(payload);
