...
function ActiveAccountHeader({ account, onAction, onMenuOpenChange }: { account: Account; onAction: (action: string) => void; onMenuOpenChange: (open: boolean) => void }) {
...
export function CentralWhatsAppApp() {
  ...
  useEffect(() => {
    ...
    desktopBridge.setWhatsAppVisible(!overlayOpen);
    return () => { if (overlayOpen) desktopBridge.setWhatsAppVisible(true); };
  }, [addOpen, settingsOpen, edit]);
