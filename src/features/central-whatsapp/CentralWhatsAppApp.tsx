...
function ActiveAccountHeader({ account, onAction }: { account: Account; onAction: (action: string) => void }) {
  return <header className="flex h-[74px] shrink-0 items-center justify-between border-b border-border bg-surface px-5">
    ...
    <div className="flex items-center gap-1">
      ...
      <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" aria-label="Mais opções"><EllipsisVertical /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-52"><DropdownMenuItem onSelect={() => onAction("rename")}>Renomear conta</DropdownMenuItem><DropdownMenuItem onSelect={() => onAction("color")}>Alterar cor</DropdownMenuItem><DropdownMenuItem onSelect={() => onAction("reorder")}>Reordenar contas</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => onAction("install-waseller")}>Instalar WaSeller nesta conta</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => onAction("disconnect")}>{account.connectionStatus === "connected" ? "Desconectar" : "Conectar"}</DropdownMenuItem><DropdownMenuItem className="text-destructive focus:text-destructive" onSelect={() => onAction("delete")}><Trash2 />Excluir conta</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
    </div>
  </header>;
}
...
export function CentralWhatsAppApp() {
  ...
  useEffect(() => setDesktopMode(desktopBridge.isDesktop), []);
  ...
  useEffect(() => desktopBridge.onUnread(({ accountId, count }) => {
